...
private:
	template <typename T>
	static void merge_sort_loop(std::vector<T>& vec, std::vector<T>& temp, bool (*comparison_fcn)(const T&, const T&), int begin, int end);
	template <typename T>
	static void merge(std::vector<T>& vec, std::vector<T>& temp, bool (*comparison_fcn)(const T&, const T&), int begin, int middle, int end);
public:
	template <typename T>
